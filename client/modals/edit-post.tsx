...
import { X, Smile, MapPin, ImagePlus, LoaderCircle } from "lucide-react";
...
const EditPost = ({ children, isOpen, setIsOpen, post }: EditPostProps) => {
  ...
  const createPost = async (imageUrl: string) => {
    ...
    const localApiResponse = await putRequest(`/post/${post._id}`, payload);
    ...
    <AlertDialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <AlertDialogContent className="!ring-0 !outline-none h-auto px-0 pt-0 py-0 gap-3 w-full max-w-xl rounded-md">
        <AlertDialogTitle className="flex justify-between items-center border-b px-4 py-2">
          <h4 className="flex-grow text-center">Edit post</h4>
          <AlertDialogCancel
            ref={cancelRef}
